...
import PositionedMenu from "./TransactionExport";
...
const TransactionDetailsTable = ({
    ...
    const [excelToekn, setExcelToken] = useState('');
    ...
    const csv = async () => {
        try {
            ...
            downloadCSV(response.data.data.Token, filters, "csvriskaccdoc", "transactiongl", "GL")
        }
        catch (err) {
            console.error(err);
        }
    ...
    const excel = async () => {
        try {
            ...
            downloadExcel(response.data.data.Token, filters, "riskaccdoc", "transactiongl", "GL")
        }
        catch (err) {
            console.error(err);
        }
    ...
    const handleChangePage = (event: unknown, newPage: number) => {
        ...
        setPage(newPage);
        handlePageChanges(event,newPage+1);
    };

    const exportButnClick = (event: React.MouseEvent<HTMLButtonElement>) => {
        setAnchorEl(event.currentTarget);
    };
    ...
    return (
        <div className="flex flex-col grow w-full">
            <div className="flex w-full shadow-lg border border-solid border-slate-300 rounded-lg">
                                                        ...
                                                        <Button
                                                            onClick={
                                                                (event) => {
                                                                    ...
                                                                    setNewTransactionId(row.transactionId);
                                                                }
                                                            }
                                                        >
                                                            <Badge
