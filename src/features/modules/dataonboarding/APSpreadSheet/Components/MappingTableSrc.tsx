...
export default function SourceMappingTable(props: Props) {
    ...
    const [tableRows1, setTableRows1] = React.useState<any[]>([]);
    const [activeStep, setActiveStep] = React.useState(0);

    const parseData = (data: any) => {
        const rows: any[] = [];
        const dateColumns = [
            { columnName: "Entry Date", dateFormat: "MM-DD-YYYY" },
        ];
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const row2 = rowCount > 1 ? props.srcrows[1][props.srcheaders[i]]:"";
            let dataType = typeof row1;
            let extra = "string";
            const dateFormat = dateColumns.find(
                (col) => col.columnName === props.srcheaders[i]
            )?.dateFormat;
    ...
    const parseData1 = (data: any) => {
        const rows: any[] = [];
        const dateColumns = [
          { columnName: "Entry Date", dateFormat: "MM-DD-YYYY" },
      ];
        const rowCount=props.srcrows1.length ;
        //console.log(rowCount, "row count")
        for (let i = 0; i < data.length; i++) {
            const name = props.srcheaders1[i];
            //const row1 = props.srcrows1[0][props.srcheaders1[i]];
            //const row2 = props.srcrows1[1][props.srcheaders1[i]];
            const row1 = rowCount > 0 ? props.srcrows1[0][props.srcheaders[i]] :"";
            const row2 = rowCount > 1 ? props.srcrows1[1][props.srcheaders[i]]:"";
            let dataType = typeof row1;
            let extra = "string";
            const dateFormat = dateColumns.find(
                (col) => col.columnName === props.srcheaders[i]
                    ...
                    <TableBody>
                        {tableRows.map((row) => (
                            ...
                            >
                                ...
                                <TableCell align="left">{row.row1}</TableCell>
