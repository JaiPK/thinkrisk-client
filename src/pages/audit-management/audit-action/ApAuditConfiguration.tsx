...
const ApAuditConfiguration: React.FC<ConfigurationProps> = forwardRef(
  ({ auditId, clientId }, ref) => {
    ...
    const [duplicateInvoiceThreshold, setDuplicateInvoiceThreshold] = useState<any>(null);
    const min = 1;
    const max = 9;
      ...
      for (let i = 0; i < updateModuleConfig.length; i++) {
        switch (updateModuleConfig[i].KEYNAME) {
          ...
          case "date_Sequence":
            ...
            break;
          case "duplicate_invoice_threshold":
            updateModuleConfig[i].KEYVALUE = duplicateInvoiceThreshold;
            break;
          default:
            break;
        }
          ...
          for (let i = 0; i < dataRecord.length; i++) {
            switch (dataRecord[i].KEYNAME) {
              ...
              case "date_Sequence":
                ...
                break;
              case "duplicate_invoice_threshold":
                setDuplicateInvoiceThreshold(JSON.parse(dataRecord[i].KEYVALUE))
                break;
              default:
                break;
            }
        ...
        >
          ...
          >
            ...
            <Tab sx={{ alignItems: 'start', textAlign: 'start' }} label={
              <>
                <div>
                  11. Duplicate Invoice Threshold
                  <Tooltip title="Configure the thresholds for Duplicate Invoices." placement="top">
                    <InfoIcon style={{ fontSize: '16px', color: '#1976d2' }} />
                  </Tooltip>
                </div>
              </>
          ...
          <TabPanel value={value} index={10}>
            <h4>Duplicate Invoice Threshold</h4>
            <FormControl error variant="standard">
              <div className="flex flex-wrap justify-center gap-x-2 gap-y-2">
                <TextField
                  sx={{ width: "200px", m: 3 }}
                  id="ai-score"
                  value={duplicateInvoiceThreshold || ""}
                  label="Threshold"
                  variant="standard"
                  type="number"
                  inputProps={{
                  ...
                  onChange={(e) => {
                    setDuplicateInvoiceThreshold(e.target.value);
                  }}
                />
              </div>
            </FormControl>
...
export default ApAuditConfiguration;
